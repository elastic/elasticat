...
func (c *Client) executeESQLQuery(ctx context.Context, query string) (*traces.ESQLResult, error) {
	...
	if res.StatusCode >= 400 {
		...
		// Pretty-print the query for error messages
		var prettyQuery bytes.Buffer
		json.Indent(&prettyQuery, []byte(query), "", "  ")
		return nil, fmt.Errorf("ES|QL query failed: %s\nError: %s\n\nQuery:\n%s", res.Status, string(bodyBytes), prettyQuery.String())
	}
...
func (c *Client) GetTransactionNamesESQL(ctx context.Context, lookback, service, resource string) ([]traces.TransactionNameAgg, error) {
	...
	serviceFilter := ""
	if service != "" {
		serviceFilter = fmt.Sprintf("AND service.name == \"%s\"", service)
	}

	resourceFilter := ""
	if resource != "" {
		resourceFilter = fmt.Sprintf("AND resource.attributes.deployment.environment == \"%s\"", resource)
	}
...
| SORT tx_count DESC
| LIMIT 100`,
		c.index, timeFilter, serviceFilter, resourceFilter)

	statsResult, err := c.executeESQLQuery(ctx, q1)
	if err != nil {
...
| LIMIT 100000`,
	...
	mappingResult, err := c.executeESQLQuery(ctx, q2)
	if err != nil {
...
| STATS span_count = COUNT(*) BY trace.id`,
	...
	spanResult, err := c.executeESQLQuery(ctx, q3)
	if err != nil {
...
func (c *Client) getPerspective(ctx context.Context, lookback string, field string) ([]perspectives.PerspectiveAgg, error) {
	...
	if res.IsError() {
		...
		// Pretty-print the query for error messages
		var prettyQuery bytes.Buffer
		json.Indent(&prettyQuery, buf.Bytes(), "", "  ")
